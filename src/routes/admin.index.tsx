...
function AdminDashboardScreen() {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const daySalesMap: Record<string, number> = { Mon: 0, Tue: 0, Wed: 0, Thu: 0, Fri: 0, Sat: 0, Sun: 0 };
      ...
      const productStats: Record<string, { name: string; sold: number; revenue: number; image: string }> = {};
  ...
  if (isLoading) {
    return <div className="p-16 text-center text-muted-foreground font-medium">Synchronizing Realtime Store Data...</div>;
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-5">
        {statCards.map((card, idx) => (
          <div key={idx} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex items-center gap-4 transition hover:shadow-md">
            <div className={`w-14 h-14 rounded-2xl ${card.bg} ${card.text} flex items-center justify-center shrink-0`}>
              <card.icon className="size-6 stroke-[2]" />
            </div>
            <div className="min-w-0 flex-1">
              <span className="text-xs font-bold text-slate-400 block mb-0.5">{card.title}</span>
              <span className="text-2xl font-black text-slate-800 tracking-tight block truncate">{card.value}</span>
          ...
          <div className="bg-white rounded-3xl p-5 md:p-6 border border-slate-100 shadow-sm">
            ...
            <div className="divide-y divide-slate-50">
              ...
              {stats?.topProducts?.length === 0 && (
                <div className="py-8 text-center text-xs text-slate-400">No product inventory dispatch logs found.</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-5 md:p-6 border border-slate-100 shadow-sm">
            ...
            <div className="divide-y divide-slate-50">
              {stats?.recentOrders?.map((ord: any, idx: number) => {
                ...
                const isProcessing = ord.status?.toLowerCase() === "processing" || ord.status?.toLowerCase() === "shipped" || idx % 3 === 1;
                
                const statusLabel = ord.status || (isCompleted ? "Completed" : isProcessing ? "Processing" : "Pending");
                ...
                return (
                  <div key={ord._id || idx} className="py-3.5 first:pt-0 last:pb-0 flex items-center justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <span className={`text-[9px] font-bold px-2 py-0.5 rounded-md inline-block uppercase tracking-wider mt-0.5 ${badgeStyle}`}>
                        {statusLabel}
                      </span>
                    </div>
                  </div>
