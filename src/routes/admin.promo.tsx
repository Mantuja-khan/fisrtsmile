...
function AdminPromo() {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground mt-0">These banners will appear side-by-side right below the categories on the home page.</p>

      {showForm && (
        <form onSubmit={save} className="bg-surface rounded-xl shadow-card p-4 space-y-3 relative mt-4">
          ...
          <h3 className="font-semibold text-lg">{editingBanner ? "Edit Promo Banner" : "New Promo Banner"}</h3>
        ...
        {banners.map((b) => (
          <div key={b._id} className="p-4 flex items-center gap-4 hover:bg-muted/30 transition">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => startEdit(b)} className="p-2 hover:bg-primary/10 rounded-full transition" title="Edit Promo Banner">
                <Pencil className="size-4 text-primary" />
              </button>
              <button onClick={() => remove(b._id)} className="p-2 hover:bg-destructive/10 rounded-full transition" title="Delete Promo Banner">
                <Trash2 className="size-4 text-destructive" />
              </button>
            </div>
          </div>
        ))}
        {banners.length === 0 && <div className="p-12 text-center text-sm text-muted-foreground">No promo banners added yet</div>}
      </div>
    </div>
  );
}
