    ...
    <div className="mt-8 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="p-4 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center pb-8 border-b border-slate-200">
          ...
          <div className="flex flex-col items-center border-slate-200 md:border-r px-4 text-center">
            <div className="flex items-center justify-center gap-0.5 mb-1.5">
              ...
              <span className="ml-2 text-lg font-   text-slate-700">{avg > 0 ? avg.toFixed(2) : "0.00"} out of 5</span>
            </div>
          ...
          <div className="mt-6 mb-8 bg-slate-50 border border-teal-100 rounded-xl p-5">
            {user ? (
              <form onSubmit={(e) => { submit(e); setShowForm(false); }} className="space-y-4">
                <div className="font-   text-slate-800 mb-1">{myReview ? "Edit your feedback" : "Create a Review"}</div>
                <div>
                  <div className="text-xs font-   text-slate-500 uppercase tracking-wider mb-1">Rating</div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div>
                    <label className="block text-xs font-   text-slate-500 uppercase mb-1">Title</label>
                ...
                <div>
                  <label className="block text-xs font-   text-slate-500 uppercase mb-1">Review Body</label>
                ...
                <div className="flex gap-2">
                  <button disabled={busy} className="bg-[#0D9488] text-white px-6 py-2 rounded font-   text-sm hover:bg-[#0F766E] disabled:opacity-50">
                    {busy ? "Posting..." : "Submit Feedback"}
                  </button>
                  <button type="button" onClick={() => setShowForm(false)} className="border border-slate-300 text-slate-600 px-4 py-2 rounded font-semi   text-sm">Cancel</button>
                </div>
              </form>
            ) : (
              <div className="text-center py-4 text-slate-600">
                Please <a href="/account" className="font-   text-teal-600 underline">sign in</a> first to post reviews.
              </div>
            )}
          </div>
        )}
        ...
        <div className="mt-8 mb-4 pb-2 border-b border-slate-100">
          <span className="text-teal-700 font-semi   border-b-2 border-teal-600 pb-2">Most Recent</span>
        </div>
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="bg-teal-700 text-white text-[10px] px-1.5 py-0.5 rounded-sm font-   uppercase tracking-wide scale-90">Verified</span>
                    </div>
                  </div>
                </div>

                {/* User Feedback Text */}
                {r.title && <h4 className="font-extra   text-slate-800 text-[15px] mb-1">{r.title}</h4>}
